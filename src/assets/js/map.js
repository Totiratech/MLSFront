...
function initMap() {
    ...
    $(document).ready(function() {
        ...
        function run_list_update() {
            setTimeout(() => {
                window.requestAnimationFrame(() => { run_list_update() });
                if (change_flag && parseInt($('#total').text()) != markers_info2.length) {
                    /*                     $('#bar-container').show();
                                        $('#grid-view').html('');
                                        $('#list-view').html('');
            ...
            if (markers_data.length > 0) {
                ...
                markers = markers_data.map((location) => {
                    ...
                    const marker = new google.maps.Marker({
                        position: position,
                        //label: '$' + nFormatter(location.Orig_dol, 1),
                        label: {
        ...
        $('#search-btn').click((event) => {
            ...
            if ($('#type').val() != 'commercialproperty') {
                ...
                delete search_options['bathrooms'];
                delete search_options['bedrooms'];
                delete search_options['bld_type'];
                search_options['bld-size'] = $('#amount_three').val().replaceAll(' ', '').replace('sqft', '');
                search_options['lnd-size'] = $('#amount_one').val().replaceAll('+ acres', '') + '-0';
            }
            if (search_location !== null) {
        ...
        $(document).on('click', '.pagination-item', function() {
            ...
            get_list(JSON.stringify(page_markers_data), $(this).data('type'), $(this).data('page'));
        });
