...
function initMap() {
    ...
    google.maps.event.addListener(markerClusterer, 'clusterclick', function(cluster) {
        if ($(window).width() < 1200) {
            window.scrollTo({ top: $('.map-sidebar-content').offset().top, behavior: 'smooth' });
        }
    });
    ...
    $(document).ready(function() {
        ...
        function set_pagination(props) {
            ...
            if (pages > 6) {
                $('#pagination-holder').html(`
                <li class="page-item ` + ((currnt_page == 0) ? 'd-none' : ' ') + ` "  ><a class="page-link" ` + ((currnt_page != 0) ? 'data-page="0"' : ' ') + `>First</a></li>
                        <li class="page-item ` + (((currnt_page - 1) <= 0) ? 'd-none' : ' ') + `">
                          <a class="page-link" data-page="` + (currnt_page - 1) + `" href="#">` + (currnt_page) + `</a>
                        </li>
                        <li class="page-item">
                          <a class="page-link" data-page="` + (currnt_page + 1) + `" href="#">` + (currnt_page + 1) + `</a>
                        </li>
                        <li class="page-item">
                          <a class="page-link" data-page="` + (currnt_page + 2) + `" href="#">` + (currnt_page + 2) + `</a>
                        </li>
                        <li class="page-item">
                          <a class="page-link ` + ((currnt_page == pages) ? 'd-none' : ' ') + `" href="#" data-page="` + (pages) + `" >Last</a>
                        </li>                
                `);
            }
        }
        ...
        function get_position(position) {
            if ((search_options['text'] === undefined || search_options['text'] == '') && (search_options['Area'] === undefined || search_options['Area'] == '')) {
        ...
        const autocomplete = new google.maps.places.Autocomplete(document.getElementById('search-text'), {
        ...
        function setMarkers(markers_data, type) {
            ...
            if (markers_data.length > 0) {
                ...
                markers = markers_data.map((location) => {
                    ...
                    marker.setValues({ type: type, id: location.id, ml_num: location.Ml_num, S_r: location.S_r, image: img_url + '/' + location.Ml_num + '/0.jpg', address: address, beds: location.Br, bathrooms: location.Bath_tot, price: location.Orig_dol });
                    ...
                    google.maps.event.addListener(marker, 'mouseover', async function() {
                        ...
                        let content = '<a href="' + $('#show_link').val() + show_type + '/' + this.ml_num + '" target="_blank"><div id="infoWindowBox"><img class="info_box_img" src="' + this.image + '" onerror="this.onerror=null; this.src=\'images/fp-1.jpg\'"/>' +
                            '<div class="info_box_body"><div class="info_box_price">' +
                            '<b>' + parseFloat(this.price).toLocaleString('en-US', {
                                style: 'currency',
                                currency: 'USD',
                            }) + '</b></div>' +
                            '<div class="info_box_address"><span>' + this.address + '</span></div><div class="info_box_icons">';
                        this.beds !== undefined && parseInt(this.beds) != 0 && (content += '<div><i class="fas fa-bed"></i><span>' + this.beds + '</span></div>');
                        this.bathrooms !== undefined && parseInt(this.bathrooms) != 0 && (content += '<div><i class="fas fa-bath"></i><span>' + this.bathrooms + '</span></div>');
                        content += '</div></div></div></a>';
                        infowindow.setContent(content);
                        infowindow.open(map, marker);
                    });
                    marker.addListener('mouseout', function() {
                        closeInfoWindowWithTimeout = setTimeout(() => infowindow.close(map, marker), 500);
                    });
        ...
        $('#search-btn').click((event) => {
            ...
            search_options['property_status'] = $('input[name="search_type"]:checked').val();
            if ($('#property-status').val() != 'Lease' && $('#max-price').val() != '') {
            ...
            if (search_address === null || !search_address.hasOwnProperty('geometry')) {
                ...
                markers.map((marker) => {
                    if (map.getBounds().contains(marker.getPosition())) {
                        markers_info[i] = { id: marker.id, lat: marker.getPosition().lat(), lng: marker.getPosition().lng() }
                        i++;
                    }
                });
        ...
        function get_list(list_markers, type, page) {
            ...
            const list_data = JSON.parse($.ajax({
                url: find_url + '/list',
                method: 'post',
